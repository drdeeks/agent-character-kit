# AGENTS.md — Agent Character Kit (ACK) v1.0.5
...
## Version tracking
`VERSION` at repo root = `1.0.5`. The daemon prints it on boot
(`ACK Enforcer daemon v1.0.5`) and returns it in `heartbeat.version`. The Hermes
